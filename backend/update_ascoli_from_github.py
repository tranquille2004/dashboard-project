#!/usr/bin/env python3
"""
Update L'Ascoli data from GitHub repository
Source: https://github.com/tranquille2004/Ascoli
"""
import asyncio
...
# Full gallery from GitHub repo (Gallery.jsx)
GITHUB_GALLERY_IMAGES = [
...
async def update_gallery_from_github():
    """Update L'Ascoli gallery with all images from GitHub repo"""
    site_id = "site_ascoli001"
    now = datetime.now(timezone.utc).isoformat()
    
    # Delete existing gallery
    print("Verwijderen bestaande galerij...")
    await db.gallery_images.delete_many({"site_id": site_id})
    
    # Insert all images from GitHub
    print(f"Toevoegen {len(GITHUB_GALLERY_IMAGES)} foto's uit GitHub repo...")
    gallery_docs = []
    for i, url in enumerate(GITHUB_GALLERY_IMAGES):
        gallery_docs.append({
            "image_id": f"img_ascoli_gh_{i:03d}",
            "site_id": site_id,
            "url": url,
    ...
    print(f"Toegevoegd: {len(gallery_docs)} foto's uit GitHub repo tranquille2004/Ascoli")

async def main():
    print("=== Update L'Ascoli data vanuit GitHub ===")
    print("Bron: https://github.com/tranquille2004/Ascoli")
    print()
    await update_gallery_from_github()
    print("\n=== Klaar! ===")
    client.close()
